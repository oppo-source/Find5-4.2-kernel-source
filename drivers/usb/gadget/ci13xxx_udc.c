...
static int _hardware_enqueue(struct ci13xxx_ep *mEp, struct ci13xxx_req *mReq)
{
    ...
    wmb();//added by songxh for usb bus patch 
	/* Remote Wakeup */
	if (udc->suspended) {
		if (!udc->remote_wakeup) {
...
static int _hardware_dequeue(struct ci13xxx_ep *mEp, struct ci13xxx_req *mReq)
{
	...
	if (mReq->zptr) {
		if ((TD_STATUS_ACTIVE & mReq->zptr->token) != 0)
			return -EBUSY;
		//added by songxh for usb bus patch begain
		...
		*/
		...
		//added by songxh for usb bus patch end
		mReq->zptr = NULL;
	}
...
__acquires(udc->lock)
{
	...
	if (retval)
		goto done;
	//deleted by songxh for usb bus patch 
...
static void isr_get_status_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	//modified by songxh for usb bus patch 
	//kfree(req->buf);
