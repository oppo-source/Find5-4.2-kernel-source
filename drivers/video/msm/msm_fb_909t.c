...
/* OPPO 2013-09-5 gousj Add begin for bkl adjust */
#if 0
		...
		if(bk_device_3630 == false && bk_device_3528 == true)
		{
			bl_lvl = get_bright_level(value);
			pr_debug("%s: Neal 3528 set bkl level = %d\n", __func__,bl_lvl);
		}
		else
			{
				bl_lvl = mfd->panel_info.bl_min + ((value - 1) * 2 *
				...
				(MAX_BACKLIGHT_BRIGHTNESS - 1) / 2;
				bl_lvl = value;
				pr_debug("%s: Neal 3630 set bkl level = %d\n", __func__,bl_lvl);
			}
#endif
/* OPPO 2013-09-5 gousj Add end */
						...
						struct mdp_bl_scale_data *data)
{
	...
	//pr_info("%s Neal curr_bl = %d",__func__,curr_bl);
	up(&mfd->sem);
