...
int lm3630_bkl_control(unsigned char bkl_level)
{
    ...
    if(sleep_mode == true)
    {
		...
		/*OPPO 2013-11-18 zhzhyon Add for CABC when PVT4 and normal mode enable PWM*/
        if((get_boot_mode() == MSM_BOOT_MODE__NORMAL) && (get_pcb_version() == PCB_VERSION_PVT4_TD))
        {
        	printk(KERN_INFO "The PVT4 version:enable PWM\n");
